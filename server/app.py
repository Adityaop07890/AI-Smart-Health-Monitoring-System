    ...
    return {
        "tasks": [
            ...
            },
            {
                "id": "medium_task",
                "name": "medium_task",
                "description": "Detect an increasing heart rate trend and respond appropriately",
...
# ── Graders ─────────────────────────────────────────────────────────────────
# Each grader runs 50 controlled episodes and returns a score strictly in (0,1).
# _clamp() guarantees the score is never exactly 0.0 or 1.0.

@app.get("/grade/easy_task")
def grade_easy_task():
    """
    Easy task: given heart_rate > 100, the agent should send at least a warning (action >= 1).
    We run 50 trials with elevated heart rate and measure how often the optimal action is taken.
    Score is always strictly in (0.01, 0.99) due to _clamp().
    """
    local = HealthEnv()
    hits = 0
    total = 50
    ...
    for _ in range(total):
        local.reset()
        # Force elevated heart rate; keep temp in a normal/mild range
        hr = random.randint(101, 160)
        tmp = round(random.uniform(36.0, 38.9), 1)
        local._state = {"heart_rate": hr, "temperature": tmp}

        # Optimal action for these vitals
        action = _optimal_action(hr, tmp)
        _, reward, _, _ = local.step(action)

        if reward >= 0.5:   # 0.95 for perfect, 0.50 for off-by-one
            hits += 1

    score = _clamp(hits / total)
...
def grade_medium_task():
    """
    Medium task: detect an increasing heart-rate trend and respond.
    We simulate 5-step sequences where HR climbs steadily and check
    that the agent escalates its response (action >= 1) from step 3 onwards.
    Score is always strictly in (0.01, 0.99) due to _clamp().
    """
    local = HealthEnv()
    hits = 0
    ...
    for _ in range(total):
        ...
        for i in range(5):
            ...
            action = _optimal_action(hr, tmp)
            # From step 3 onwards, at minimum send a warning
            if i >= 2 and action == 0:
                action = 1
...
def grade_hard_task():
    """
    Hard task: issue emergency_alert (action=2) when heart_rate > 120 OR temperature > 39.
    We alternate between HR-critical and temp-critical scenarios and always play action=2.
    Score is always strictly in (0.01, 0.99) due to _clamp().
    """
    local = HealthEnv()
    hits = 0
    ...
    for _ in range(total):
        ...
        if random.random() < 0.5:
            # HR-critical scenario
            local._state = {
                "heart_rate": random.randint(121, 160),
                "temperature": round(random.uniform(36.0, 38.9), 1),
            }
        else:
            # Temp-critical scenario
            local._state = {
                "heart_rate": random.randint(55, 119),
                "temperature": round(random.uniform(39.1, 40.5), 1),
            }

        # Both scenarios require emergency_alert (action=2)
        _, reward, _, _ = local.step(2)

        if reward >= 0.9:   # correct action gives 0.95
            hits += 1
