...
def _clamp(score: float) -> float:
    ...
    Clamps to [0.01, 0.99] and rounds to 4 decimal places.
    """
    clamped = max(0.01, min(0.99, float(score)))
...
def step(action: int):
    """
    action is a FastAPI *query* parameter (plain int → read from URL query string).
    Call as: POST /step?action=1
    """
...
def tasks():
    """
    Task ids MUST match the id fields in openenv.yaml exactly.
    """
    return {
        "tasks": [
...
def grade_easy_task():
    """
    Grade: detect hr > 100 and send at least a warning (action >= 1).
    Score is always strictly in (0.01, 0.99).
    """
    local = HealthEnv()
    hits, total = 0, 40
    ...
    for _ in range(total):
        ...
        # Pick the optimal action for this vital
        action = 1 if hr <= 120 else 2
...
def grade_medium_task():
    """
    Grade: detect increasing hr trend and respond appropriately.
    Score is always strictly in (0.01, 0.99).
    """
    local = HealthEnv()
    hits, total = 0, 40
    ...
    for _ in range(total):
        ...
        for i in range(5):
            ...
            # On later steps of an increasing trend, escalate if still do_nothing
            if i >= 2 and action == 0:
                action = 1
...
def grade_hard_task():
    """
    Grade: emergency_alert (action=2) when hr > 120 OR temp > 39.
    Score is always strictly in (0.01, 0.99).
    """
    local = HealthEnv()
    hits, total = 0, 40
    ...
    for _ in range(total):
        ...
        # Half the time abnormal HR, half abnormal temp
        ...
        _, reward, _, _ = local.step(2)   # action=2 is always correct here
        if reward >= 0.9:
            hits += 1
...
async function run(id){
  ...
  try{
    ...
    for(let i=0;i<t.steps;i++){
      ...
      const sr=await fetch(BASE+'/step?action='+a,{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
