...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .btn-0:hover { border-color: var(--green); background: rgba(0,230,118,0.06); }
  .btn-1:hover { border-color: var(--warn); background: rgba(255,179,0,0.06); }
  .btn-2:hover { border-color: var(--accent2); background: rgba(255,56,96,0.06); }
  ...
  .ctrl-btn {
    ...
    background: transparent; color: var(--text); font-family: var(--sans); font-size: 0.82rem;
    font-weight: 600; cursor: pointer; letter-spacing: 0.05em; transition: all 0.2s;
  }
  .ctrl-btn.primary { border-color: var(--accent); color: var(--accent); }
  .ctrl-btn.primary:hover { background: rgba(0,229,255,0.1); }
  .ctrl-btn.secondary:hover { border-color: var(--muted); background: rgba(255,255,255,0.03); }
  ...
  .done-banner p { color: var(--muted); font-family: var(--mono); font-size: 0.85rem; }
    ...
    <div>
      ...
      <div class="stats-row">
        <div class="stat-mini">
          ...
          <div class="val" id="step-val">0 / 20</div>
        </div>
        <div class="stat-mini">
          <div class="label">Total Reward</div>
          <div class="val" id="reward-val">0.0000</div>
        </div>
        <div class="stat-mini">
          <div class="label">Avg Reward</div>
          <div class="val" id="avg-reward-val">—</div>
        </div>
      </div>
    </div>
    ...
    <div class="card control-panel">
      ...
      <div class="action-btns" id="action-btns">
        <button class="action-btn btn-0 disabled-state" onclick="takeAction(0)">
          ...
          <div>
            <div>Do Nothing</div>
            <div class="btn-sub">Vitals are normal</div>
          </div>
        </button>
        <button class="action-btn btn-1 disabled-state" onclick="takeAction(1)">
          <div class="btn-icon">⚠</div>
          <div>
            <div>Send Warning</div>
            <div class="btn-sub">Mildly concerning vitals</div>
          </div>
        </button>
        <button class="action-btn btn-2 disabled-state" onclick="takeAction(2)">
          ...
          <div>
            ...
            <div class="btn-sub">Critical vitals detected</div>
          </div>
        </button>
      </div>
...
let ecgData = new Array(ECG_POINTS).fill(0);
let ecgAnimId;

function ecgWave(t) {
  const tp = ((t % 60) / 60);
  if (tp < 0.05) return Math.sin(tp / 0.05 * Math.PI) * 0.15;
  if (tp < 0.1) return -Math.sin((tp - 0.05) / 0.05 * Math.PI) * 0.05;
  if (tp < 0.2) return 0;
  if (tp < 0.22) return Math.sin((tp - 0.2) / 0.02 * Math.PI) * 0.3;
  if (tp < 0.24) return -Math.sin((tp - 0.22) / 0.02 * Math.PI);
  if (tp < 0.26) return Math.sin((tp - 0.24) / 0.02 * Math.PI) * 1.4;
  if (tp < 0.28) return -Math.sin((tp - 0.26) / 0.02 * Math.PI) * 0.3;
  if (tp < 0.35) return Math.sin((tp - 0.28) / 0.07 * Math.PI) * 0.2;
  if (tp < 0.42) return Math.sin((tp - 0.35) / 0.07 * Math.PI) * 0.2;
  return 0;
}
...
function drawECG() {
  ...
  ecgOffset += 0.8;
  ecgData.shift();
  ecgData.push(ecgWave(ecgOffset));

  const grad = ecgCtx.createLinearGradient(0, 0, w, 0);
  ...
  for (let i = 0; i < ECG_POINTS; i++) {
    const x = (i / ECG_POINTS) * w;
    const y = h / 2 - ecgData[i] * (h * 0.38);
...
function drawHRChart() {
  ...
  if (hrHistory.length < 2) {
    hrCtx.fillStyle = 'rgba(74,112,128,0.4)';
    hrCtx.font = '0.72rem Share Tech Mono, monospace';
    hrCtx.textAlign = 'center';
    hrCtx.fillText('No data yet — reset to start', w / 2, h / 2);
    return;
  }

  const min = Math.min(...hrHistory, 55) - 5;
  const max = Math.max(...hrHistory, 160) + 5;
  const pad = { t: 10, b: 28, l: 36, r: 10 };
  const cw = w - pad.l - pad.r, ch = h - pad.t - pad.b;

  const toX = i => pad.l + (i / (hrHistory.length - 1)) * cw;
  const toY = v => pad.t + (1 - (v - min) / (max - min)) * ch;

  // grid lines
  hrCtx.strokeStyle = 'rgba(26,48,64,0.8)';
  hrCtx.lineWidth = 1;
  [60, 80, 100, 120, 140].forEach(v => {
    const y = toY(v);
    hrCtx.beginPath();
    hrCtx.moveTo(pad.l, y); hrCtx.lineTo(w - pad.r, y);
    hrCtx.stroke();
    hrCtx.fillStyle = 'rgba(74,112,128,0.7)';
    hrCtx.font = '9px Share Tech Mono';
    hrCtx.textAlign = 'right';
    hrCtx.fillText(v, pad.l - 4, y + 3);
  });

  // danger zone
  const dangerY = toY(120);
  hrCtx.fillStyle = 'rgba(255,56,96,0.05)';
  hrCtx.fillRect(pad.l, pad.t, cw, dangerY - pad.t);

  // line fill
  const grad = hrCtx.createLinearGradient(0, pad.t, 0, h - pad.b);
  grad.addColorStop(0, 'rgba(0,229,255,0.18)');
  grad.addColorStop(1, 'rgba(0,229,255,0)');
  hrCtx.beginPath();
  hrHistory.forEach((v, i) => i === 0 ? hrCtx.moveTo(toX(i), toY(v)) : hrCtx.lineTo(toX(i), toY(v)));
  hrCtx.lineTo(toX(hrHistory.length - 1), h - pad.b);
  hrCtx.lineTo(toX(0), h - pad.b);
  hrCtx.closePath();
  ...
  // line
  hrCtx.beginPath();
  hrCtx.strokeStyle = '#00e5ff';
  hrCtx.lineWidth = 2;
  hrCtx.shadowColor = '#00e5ff';
  hrCtx.shadowBlur = 4;
  hrHistory.forEach((v, i) => i === 0 ? hrCtx.moveTo(toX(i), toY(v)) : hrCtx.lineTo(toX(i), toY(v)));
  hrCtx.stroke();
  ...
  hrHistory.forEach((v, i) => {
    hrCtx.beginPath();
    hrCtx.arc(toX(i), toY(v), 3, 0, Math.PI * 2);
    hrCtx.fillStyle = v > 120 ? '#ff3860' : v > 100 ? '#ffb300' : '#00e5ff';
    hrCtx.shadowColor = hrCtx.fillStyle;
    hrCtx.shadowBlur = 6;
    hrCtx.fill();
  });
}
...
function log(msg, type = 'info') {
  ...
  div.innerHTML = `<span class="ts">${now}</span><span class="msg-${type}">${msg}</span>`;
  el.prepend(div);
  while (el.children.length > 60) el.removeChild(el.lastChild);
}

function flashReward(r) {
  const el = document.getElementById('reward-flash');
  el.textContent = `REWARD +${r}`;
  el.classList.add('show');
  setTimeout(() => el.classList.remove('show'), 1200);
}

function updateVitals(hr, temp) {
  ...
  if (hr > 120 || temp > 39) {
    ...
    alertVal.textContent = 'CRITICAL';
    alertSub.textContent = 'Immediate action required';
  } else if (hr > 100 || temp > 38) {
    hrCard.classList.add('warn');
    tempCard.classList.add('warn');
    alertCard.classList.add('warn');
    alertVal.textContent = 'WARNING';
    alertSub.textContent = 'Monitor closely';
  } else {
    hrCard.classList.add('normal');
    tempCard.classList.add('normal');
    alertCard.classList.add('normal');
    alertVal.textContent = 'NORMAL';
    alertSub.textContent = 'Vitals within range';
  }
}

function updateStats() {
  document.getElementById('step-val').textContent = `${stepCount} / 20`;
  document.getElementById('reward-val').textContent = totalReward.toFixed(4);
  const avg = stepCount > 0 ? (totalReward / stepCount).toFixed(4) : '—';
  document.getElementById('avg-reward-val').textContent = avg;
}

async function fetchScores() {
  const [e, m, h] = await Promise.all([
    api('GET', '/grade/easy_task'),
    api('GET', '/grade/medium_task'),
    api('GET', '/grade/hard_task'),
  ]);
  const scores = [e.score, m.score, h.score];
  ['score-0','score-1','score-2'].forEach((id, i) => {
    document.getElementById(id).textContent = scores[i].toFixed(4);
    document.getElementById(`score-bar-${i}`).style.width = `${scores[i]*100}%`;
  });
}
...
async function resetEpisode() {
  ...
  const d = await api('POST', '/reset');
  const obs = d.observation;
  hrHistory = [obs.heart_rate];
  totalReward = 0;
  stepCount = 0;
  isActive = true;

  updateVitals(obs.heart_rate, obs.temperature);
  updateStats();
  drawHRChart();
  setActionBtns(true);
  document.getElementById('status-text').textContent = 'MONITORING — EPISODE ACTIVE';
  document.getElementById('last-reward-val').textContent = '—';
  document.getElementById('done-banner').classList.remove('show');
  log(`Episode reset. Initial HR=${obs.heart_rate} BPM, Temp=${obs.temperature}°C`, 'info');
  fetchScores();
}

async function takeAction(action) {
  if (!isActive) return;
  const labels = ['DO_NOTHING', 'SEND_WARNING', 'EMERGENCY_ALERT'];
  const types = ['ok', 'warn', 'err'];
  const d = await api('POST', '/step', {action});

  totalReward += d.reward;
  stepCount += 1;
  hrHistory.push(d.observation.heart_rate);

  document.getElementById('last-reward-val').textContent = `+${d.reward}`;
  updateVitals(d.observation.heart_rate, d.observation.temperature);
  updateStats();
  drawHRChart();
  flashReward(d.reward);
  log(`Step ${stepCount}: Action=${labels[action]}, Reward=${d.reward}, HR=${d.observation.heart_rate}, Temp=${d.observation.temperature}`, types[action]);
  fetchScores();
  ...
  if (d.done) {
    ...
    setActionBtns(false);
    if (autoInterval) toggleAuto();
    document.getElementById('status-text').textContent = 'EPISODE COMPLETE';
    const avg = (totalReward / stepCount).toFixed(4);
    document.getElementById('done-summary').textContent = `Total Reward: ${totalReward.toFixed(4)}  |  Avg: ${avg}  |  Steps: ${stepCount}`;
    document.getElementById('done-banner').classList.add('show');
    log(`Episode done. Total reward=${totalReward.toFixed(4)}, Avg=${avg}`, 'info');
  }
}
...
function toggleAuto() {
  ...
  if (!isActive) { resetEpisode().then(startAuto); return; }
  startAuto();
}
...
function startAuto() {
  ...
  autoInterval = setInterval(async () => {
    if (!isActive) { toggleAuto(); return; }
    const hr = parseInt(document.getElementById('hr-val').textContent) || 75;
    const temp = parseFloat(document.getElementById('temp-val').textContent) || 37;
    let action = 0;
    if (hr > 120 || temp > 39) action = 2;
    else if (hr > 100 || temp > 38) action = 1;
...
def _clamp(x):
    return round(min(0.89, max(0.11, x)), 4)
...
def _score():
    ...
    if len(hr_hist) >= 2:
        ...
        medium = _clamp(inc / (len(hr_hist) - 1) * 0.85)
    else:
        medium = _clamp(0.5)
    hard = _clamp(avg * 0.85)
